    ...
    def optimize(self, instructions: List[Instruction]) -> List[Instruction]:
        ...
        while i < len(instructions):
            ...
            if (i + 2 < len(instructions) and
                ...
                instructions[i+1].opcode == Opcode.LOAD_CONST and
                ...
                if isinstance(left, (int, float)) and isinstance(right, (int, float)):
                    if op == Opcode.ADD:
                        ...
                        if right != 0:
                            ...
                            optimized.extend([instructions[i], instructions[i+1], instructions[i+2]])
                            i += 3
                            continue
...
class BytecodeCompilerV2:
    ...
    def _encode_instruction(self, data: bytearray, inst: Instruction):
        ...
        if inst.opcode in (Opcode.LOAD_CONST, Opcode.JMP, Opcode.JMP_IF_FALSE, Opcode.JMP_IF_TRUE, Opcode.CALL):
            # Integer argument
