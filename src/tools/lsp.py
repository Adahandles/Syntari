...
class SyntariLSP:
    ...
    def __init__(self):
        ...
        self.builtins = [
            'print', 'trace', 'exit', 'env', 'time',
            'ai.query', 'ai.eval', 'ai.suggest',
        ]
    ...
    def get_completions(self, uri: str, position: Position) -> List[CompletionItem]:
        ...
        if uri in self.symbols:
            for symbol in self.symbols[uri]:
                kind = self.COMPLETION_FUNCTION if symbol.kind == self.SYMBOL_FUNCTION else self.COMPLETION_VARIABLE
                completions.append(CompletionItem(
                    label=symbol.name,
                    kind=kind,
...
class LSPServer:
    ...
    def handle_message(self, message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if method == 'initialize':
            ...
            return {
                'id': msg_id,
                'result': [c.to_dict() for c in completions]
            }
            ...
            return {
                'id': msg_id,
                'result': {'contents': hover} if hover else None
            }
