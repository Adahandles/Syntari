...
class Profiler:
    ...
    def _generate_text_report(self) -> str:
        ...
        if self.stats.functions:
            ...
            lines.append("-" * 80)
            lines.append(f"{'Function':<30} {'Calls':>8} {'Total Time':>12} {'Avg Time':>12} {'% Total':>8}")
            lines.append("-" * 80)
            
            for func in self.stats.get_hot_functions(20):
                pct = (func.total_time / self.stats.total_execution_time * 100) if self.stats.total_execution_time > 0 else 0
                lines.append(
        ...
        if self.enable_line_profiling and self.stats.line_execution_counts:
            ...
            for lineno, count in self.stats.get_hot_lines(15):
                pct = (count / self.stats.total_instructions * 100) if self.stats.total_instructions > 0 else 0
    ...
    def _generate_json_report(self) -> str:
        ...
        report = {
            ...
            "hot_lines": [
                ...
                for line, count in self.stats.get_hot_lines(50)
            ],
            "opcodes": [
                {"opcode": op, "count": count}
                for op, count in self.stats.get_hot_opcodes(30)
            ]
        }
...
def profile_interpreter(source_file: str, output_format: str = 'text', output_file: Optional[str] = None):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m src.tools.profiler <source.syn> [--format text|json|html] [--output file]")
        print("\nOptions:")
