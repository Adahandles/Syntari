...
class SyntariVMV2:
    ...
    def load_bytecode(self, path: str):
        ...
        if len(data) > MAX_BYTECODE_SIZE:
            raise VMSecurityError(f"Bytecode size {len(data)} exceeds maximum {MAX_BYTECODE_SIZE}")
    ...
    def _debug_print(self, opcode: int, lineno: int):
        ...
        print(
            f"[VM] Line {lineno}: {opcode_name} | Stack: {self.stack[-5:] if len(self.stack) > 0 else '[]'}"
        )
